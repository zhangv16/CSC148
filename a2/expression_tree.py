...
class ExprTree:
    ...
    def __str__(self) -> str:
        ...
        if isinstance(self._root, int):
            ...
            string = '(' + str(self._subtrees[0])
            for i in range(1, len(self._subtrees)):
                string += ' ' + self._root + ' ' + str(self._subtrees[i])
            string += ')'
            return string
        else:
            return self._root
    ...
    def populate_lookup(self, lookup: Dict[str, int]) -> None:
        ...
        True
        """
        if self._root is None or isinstance(self._root, int):
            return
        elif not self._subtrees:
            if self._root not in lookup.keys():
                lookup[self._root] = 0
        else:
            for subtree in self._subtrees:
...
def construct_from_list(values: List[List[Union[str, int]]]) -> ExprTree:
    ...
    for lst in values:
        if not queue.is_empty():
            curr_root = queue.dequeue()
        for value in lst:
            new_tree = ExprTree(value, [])
            if root is None:
                root = new_tree
            else:
                curr_root.append(new_tree)
            if value in OPERATORS:
                queue.enqueue(new_tree)
    return root
