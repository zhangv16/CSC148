    ...
    def extensions(self) -> List[WordLadderPuzzle]:
        ...
        for i in range(len(self.from_word)):
            for char in self._chars:
                temp = None
                if char != self.from_word[i]:
                    temp = self.from_word[:i] + char + self.from_word[i + 1:]
                if temp and temp in self.word_set:
                    extend = WordLadderPuzzle(temp, self.to_word, self.word_set)
                    lst.append(extend)
        return lst
